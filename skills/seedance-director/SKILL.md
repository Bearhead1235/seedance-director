...
Phase 1-2 完成后，评估素材情况，按需生成缺失的视觉锚定素材。

**素材自适应检测**：

| 用户有什么 | 系统做什么 |
|-----------|-----------|
| 有角色图 + 有场景图 + 有详细描述 | 跳过素材制备，直接进入 Phase 4 |
| 有角色图，没场景图 | 只生成场景概念图和关键帧 |
| 没有角色图，有文字描述 | 生成角色三视图 + 场景概念图 + 关键帧 |
| 什么都没有，只有一个想法 | 完整走：角色三视图 → 场景图 → 关键帧 |
...
- **单段**：输出 2-3 个版本（简洁版 / 详细版 / 创意变体）+ 操作指引
- **2 段**：逐段输出，段2 用视频延长
