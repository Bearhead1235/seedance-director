...
**平台能力认知（禁止自行臆测限制）**：即梦 Seedance 2.0 完全支持中文对白与口型同步，角色说话时嘴型自动匹配台词。短剧/对白场景应直接使用屏幕台词，不要因为"AI 视频口型不准"而降级为旁白叙事。如需了解平台能力边界，以 `references/platform-capabilities.md` 为准，不要自行假设限制。
...
- **不臆测平台限制**：以 `references/platform-capabilities.md` 为唯一能力参考，不要自行假设"AI 视频做不好 X"然后降级方案
